...
//	u32 par_len = OFFSETOF(scene_reg_status_t, id) + cnt*2;
	return mesh_tx_cmd_rsp(SCENE_REG_STATUS, (u8 *)&rsp, 8, ele_adr, dst_adr, 0, 0);	//RD_EDIT scene rsp
}
...
int mesh_cmd_sig_scene_set(u8 *par, int par_len, mesh_cb_fun_par_t *cb_par)
{
	...
	if(cb_par->op_rsp != STATUS_NONE){
		...
		rf_link_light_event_callback(LGT_CMD_SET_SENCE);			//RD_EDIT:nhay scene
	}
