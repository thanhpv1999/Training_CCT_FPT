...
const u32 GPIO_LED_INDEX = (GPIO_LED == PWM_R) ? 0 : ((GPIO_LED == PWM_G) ? 1 : ((GPIO_LED == PWM_B) ? 2 : ((GPIO_LED == PWM_W) ? 3 : ((GPIO_LED == PWM_W) ? 4 :0))));
...
#if (SELECT_DIM == RD_DIM_10)
    vrs_lum = 10 + lum_100*0.9;		//10%
#endif

#if (SELECT_DIM == RD_DIM_20)
